...
# 
# Update: divide by mRNA background codon counts
...
my $out = $ARGV[3];
# mRNA background: 1. codon 2. %
my $backgroundFile = $ARGV[4];

# max fp length, in codons
# 33 nt, for S. cerevisiae
my $maxLength = 10;
...
# Open the mRNA tabulations file and store in a hash
my %backgroundFreq;

open (BAK, "<$backgroundFile") or die $!;
...
pop @templist; # remove a list entry if if data are scarce
...
foreach my $c (@codonlist) {
	...
	foreach my $pos (0 .. $largestval - 1) {
		...
		my $percent = $counts{$l}[$pos]{$c} / $sums_lengths{$l}[$pos] * 100;
		my $background = $backgroundFreq{$c};
		
		print OUT $percent / $background;
	    }
	    else {
		print OUT "-";
	    }
